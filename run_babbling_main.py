...
from movementtest_functions import *

## defining the dummy plant function
def dummy_plant_fcn(x):
	y_pos=np.zeros([x.shape[0],2])
	lp_x = np.zeros(x.shape)
	for ii in range(x.shape[1]):
		lp_x[:,ii] = LP_filt(10, x[:,ii])
	y_pos[:,0]=1*lp_x[:,0]+2*lp_x[:,1]-3*lp_x[:,2] # weights are indicating some arbitary moment arm values
	y_pos[:,1]=-2*lp_x[:,0]-0*lp_x[:,1]+2*lp_x[:,2]
	y=positions_to_kinematics_fcn(y_pos[:,0], y_pos[:,1], dt=0.01)
	return y

## initialization
number_of_actions = 3 # Action space dimentions
number_of_State = 2 # State space dimentions
dt=0.01 # time step
...
babbling_signals = babbling_input_gen_fcn(
	number_of_signals=number_of_actions,
...
# here I am using a dummy plant; please change it with your plant
activations = babbling_signals
babbling_observations = dummy_plant_fcn(activations)
plt.figure()
plt.plot(babbling_observations[:,0])
plt.plot(babbling_observations[:,3])
plt.legend(['q0','q1'])
...
model=inverse_mapping_fcn(babbling_observations, activations, log_address='./log', early_stopping=False)

## running the inverse model (optional)
est_activations=model.predict(babbling_observations)
